...
def gen_coords(vm_dir = '.', debug = False, geoproj = '1', do_coords = '1', \
    ...
    try:
        ...
        print('vm dir:     %s\nsim params: %s' % (vm_dir, \
                                                  os.path.abspath(vm.__file__)))
        vm = {'nx':int(vm.nx), 'ny':int(vm.ny), 'nz':int(vm.nz), \
              'hh':float(vm.hh), 'sufx':vm.sufx, \
              'MODEL_LON':float(vm.MODEL_LON), 'MODEL_LAT':float(vm.MODEL_LAT), \
